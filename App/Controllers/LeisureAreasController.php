    ...
    public function registerEvent(){
        ...
        if(strlen($_POST['cpf']) == 14 && $_POST['titulo_evento'] != '' && $_POST['inicio_evento'] != '' && $_POST['fim_evento'] != ''){
            ...
            if($morador_existente && $fk_id_morador != false){
                ...
                if(((int)(date("H", strtotime($inicio_evento)))) > 21){
                    echo "<script>alert('A data/hora escolhida de inicio do seu evento não pode ser superior a 21:00h. Verifique!')</script>";
                    echo "<script>location.href = '/leisure_areas'</script>";
                    exit;
                }

                foreach($eventos->getAllEventsPlusDate() as $e){
                    if((date("Y-m-d H:i:s", strtotime($inicio_evento)) >= $e['inicio_evento']) && (date("Y-m-d H:i:s", strtotime($inicio_evento)) <= $e['fim_evento'])){
                        echo "<script>alert('A data/hora escolhida de inicio do seu evento precisar ser superior a ".date("d/m/Y H:i:s", strtotime($e['fim_evento']."+ 1 hours"))."!')</script>";
                        echo "<script>location.href = '/leisure_areas'</script>";
                        exit;
                    }
                }
