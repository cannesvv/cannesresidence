...
class ResidentsController extends Action {
	...
	public function residents(){
        ...
        if(($_SESSION['nivel_acesso'] == 'administrador') || ($_SESSION['nivel_acesso'] == 'porteiro')){
            ...
            $moradores = Container::getModel('Residents');
            $this->view->apartamentos = $moradores->getAllApartamentosRegisters();
            $this->view->moradores = $moradores->getAllResidentsRegistersCondomino();
    ...
    public function registerResident(){
        $this->validateAuthentication();
        if($_POST['nome'] != '' && strlen($_POST['cpf']) == 14 && $_POST['id_apartamento'] != ''){
            $moradores = Container::getModel('Residents');
            ...
            $moradores->id_apartamento = $_POST['id_apartamento'];
    ...
    public function editResident(){
        ...
        if(($_SESSION['nivel_acesso'] == 'administrador') || ($_SESSION['nivel_acesso'] == 'porteiro')){
           $this->view->apartamentos = $moradores->getAllApartamentos();
           $this->view->apartamento = $_POST['id_apartamento'];
        } else {
           $this->view->apartamentos = $moradores->getAllApartamentosRegisters();
           $this->view->apartamento = $_POST['id_apartamento'];
        }
    ...
    public function updateResident(){
        $this->validateAuthentication();
        if($_POST['nome'] != '' && strlen($_POST['cpf']) == 14 && $_POST['id_apartamento'] != '' && $_POST['id_morador'] != ''){
            $moradores = Container::getModel('Residents');
            $visitantes = Container::getModel('Visitors');
    ...
    public function exportResidents(){
		...
		$html .= '<table border="1">';
		$html .= "<tr>";
		$html .= "<td colspan='4' style='$style_first_header'><h2>Relação de moradores</h2></td>";
		$html .= "</tr>";
		$html .= '<tr>';
