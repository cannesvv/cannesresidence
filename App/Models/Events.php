...
class Events extends Model{
    ...
    public function getAllEventsPlusDate(){
        $stmt = $this->db->prepare("SELECT * FROM eventos where inicio_evento >= ".date("Y-m-d")." order by fim_evento desc");
        $stmt->execute();
        return $stmt->fetchAll(PDO::FETCH_ASSOC);
    }

    public function getAllEventAndResidentData(){
        $stmt = $this->db->prepare("SELECT eventos.*, moradores.*, apartamento.dsc_apartamento as apartamento FROM eventos 
        INNER join moradores on eventos.id_evento = :id_evento 
        inner join apartamento on moradores.id_apartamento = apartamento.id_apartamento
        and moradores.id_morador = :fk_id_morador");
        $stmt->bindValue(":id_evento", $this->id_evento);
        $stmt->bindValue(":fk_id_morador", $this->fk_id_morador);
        $stmt->execute();
        return $stmt->fetchAll(PDO::FETCH_ASSOC);
